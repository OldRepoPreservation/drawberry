...
- (NSBezierPath*)		bezierPathByInterpolatingPath:(float) amount
{
	...
	NSPoint				ap[3];
	NSPoint				v[4];
	NSPoint				fp, cp1, cp2, lp;
	BOOL				closedPath = NO;
	
	for( i = 0; i <= m; ++i )
	{
		// left-shift the vertex array
	
		v[0] = v[1]; v[1] = v[2]; v[2] = v[3];
		
		// capture info about the end of the path if necessary
		
		BOOL subPathClosed = [self subpathContainingElementIsClosed:i];
		
		if( subPathClosed )
		{
			int lastElement = [self subpathEndingElementForElement:i];
			element = [self elementAtIndex:lastElement associatedPoints:ap];
			if( element == NSCurveToBezierPathElement )
				lp = ap[2];
			else
				lp = ap[0];
		...
		if( i > 0 )
		{
			...
			[newPath curveToPoint:v[2] controlPoint1:cp1 controlPoint2:cp2];
		}
	}
...
{
	// given the vertices of the path v0..v3, this calculates cp1 and cp2 being the control points for a curve segment between v1,v2.
    ...
    float xc3 =	( v[2].x + v[3].x ) * 0.5f;	//(x2 + x3) / 2.0;
    float yc3 =	( v[2].y + v[3].y ) * 0.5f;	//(y2 + y3) / 2.0;
    ...
    float len3 = hypotf( v[3].x - v[2].x, v[3].y - v[2].y );	//sqrt((x3-x2) * (x3-x2) + (y3-y2) * (y3-y2));
    ...
    ctrl2.x = xm2 + (xc2 - xm2) * smooth_value + v[2].x - xm2;
    ctrl2.y = ym2 + (yc2 - ym2) * smooth_value + v[2].y - ym2;
...
- (NSBezierPath*)		strokedPath
{
	...
	if( wr.size.width > 0.0 && wr.size.height > 0.0 )
	{
...
static int				SortPointsHorizontally( NSValue* value1, NSValue* value2, void* context )
{
...
#if (0)
	return [self getPositionForPoint:p];
...
//		NSLog(@"position : %@ point : %f",NSStringFromPoint(p),distance);
...
static void subdivideBezier(const NSPoint bez[4], NSPoint bez1[4], NSPoint bez2[4])
{
  ...
  // Subdivide the B‚àö¬©zier further
  bez1[0].x = bez[0].x;
  bez1[0].y = bez[0].y;
  bez2[3].x = bez[3].x;
  bez2[3].y = bez[3].y;
...
// Subdivide a B‚àö¬©zier (specific division)
...
// Estimate the total length of a B‚àö¬©zier path
