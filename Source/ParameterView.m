...
//        [filterStack setActionNameForFilter:filter key:key];
    }
    // let core image recompute the display
    if (filterStack != nil)
        [filterStack filterChange];
...
- (IBAction)readoutTextFieldChanged:(id)sender
{
    ...
    if (readoutTextField != nil && slider != nil)
    {
        ...
        // update the filter (using undo-compatible glue code)
        [master setFilter:filter value:[NSNumber numberWithFloat:f] forKey:key];
        // and set up the undo string based on the filter and key names
//        [filterStack setActionNameForFilter:filter key:key];
    }
    // let core image recompute the display
...
- (IBAction)checkBoxChanged:(id)sender
{
    ...
    if (checkBox != nil)
    {
        ...
        // update the filter (using undo-compatible glue code)
        [master setFilter:filter value:[NSNumber numberWithBool:b] forKey:key];
        // and set up the undo string based on the filter and key names
//        [filterStack setActionNameForFilter:filter key:key];
    }
    // let core image recompute the display
...
- (IBAction)readout1TextFieldChanged:(id)sender
    ...
    if (readout1TextField != nil)
        ...
        // update the filter (using undo-compatible glue code)
        [master setFilter:filter value:vec2 forKey:key];
        // and set up the undo string based on the filter and key names
//        [filterStack setActionNameForFilter:filter key:key];
        }
    // let core image recompute the display
...
- (IBAction)imageWellChanged:(id)sender
{
    ...
    if (imageView != nil)
    {
        ...
        // update the filter (using undo-compatible glue code)
        [master setFilter:filter value:im forKey:key];
        // and set up the undo string based on the filter and key names
//        [filterStack setActionNameForFilter:filter key:key];
    ...
    // let core image recompute the display
/*    if (filterStack != nil)
        [filterStack setNeedsDisplay:YES];*/
...
/*- (void)synchronizeTextView:(NSTextView *)tv
{
    ...
    // save string
    [filterStack setDict:dict value:[[[tv string] copy] autorelease] forKey:@"string"];
    ts = [tv textStorage];
    dattrs = [ts attributesAtIndex:0 effectiveRange:nil];
    // save font attributes
    font = [dattrs valueForKey:NSFontAttributeName];
    [filterStack setDict:dict value:[[[font fontName] copy] autorelease] forKey:@"font"];
    [filterStack setDict:dict value:[NSNumber numberWithFloat:[font pointSize]] forKey:@"pointSize"];
    ...
    if (color == nil)
    {
        ...
        [color getRed:&red green:&green blue:&blue alpha:&alpha];
    [filterStack setDict:dict value:[NSNumber numberWithFloat:red] forKey:@"colorRed"];
    [filterStack setDict:dict value:[NSNumber numberWithFloat:green] forKey:@"colorGreen"];
    [filterStack setDict:dict value:[NSNumber numberWithFloat:blue] forKey:@"colorBlue"];
    [filterStack setDict:dict value:[NSNumber numberWithFloat:alpha] forKey:@"colorAlpha"];
    ...
    [filterStack setDict:dict value:[NSNumber numberWithBool:isShadow] forKey:@"isShadow"];
    [filterStack setDict:dict value:[NSNumber numberWithFloat:shadowOffset.width] forKey:@"shadowOffsetX"];
    [filterStack setDict:dict value:[NSNumber numberWithFloat:shadowOffset.height] forKey:@"shadowOffsetY"];
    [filterStack setDict:dict value:[NSNumber numberWithFloat:shadowColorRed] forKey:@"shadowColorRed"];
    [filterStack setDict:dict value:[NSNumber numberWithFloat:shadowColorGreen] forKey:@"shadowColorGreen"];
    [filterStack setDict:dict value:[NSNumber numberWithFloat:shadowColorBlue] forKey:@"shadowColorBlue"];
    [filterStack setDict:dict value:[NSNumber numberWithFloat:shadowColorAlpha] forKey:@"shadowColorAlpha"];
    [filterStack setDict:dict value:[NSNumber numberWithFloat:shadowBlurRadius] forKey:@"shadowBlurRadius"];
    ...
    if (strikeThroughStyle == nil)
        ...
        strikeThrough = [strikeThroughStyle intValue];
    [filterStack setDict:dict value:[NSNumber numberWithInt:strikeThrough] forKey:@"strikeThroughStyle"];
    ...
    if (underlineStyle == nil)
        ...
        underline = [underlineStyle intValue];
    [filterStack setDict:dict value:[NSNumber numberWithInt:underline] forKey:@"underlineStyle"];
    // set up an undo string for this action
    [filterStack setActionNameForTextLayerKey:@"Typing"];
}
...
/*- (IBAction)textSliderChanged:(id)sender
{
    ...
    if (readoutTextField != nil && slider != nil)
    {
        ...
        [filterStack setDict:dict value:[NSNumber numberWithFloat:f] forKey:key];
        [filterStack setActionNameForTextLayerKey:key];
...
- (IBAction)scaleSliderChanged:(id)sender
{
    ...
    if (scaleReadoutTextField != nil && scaleSlider != nil)
    {
        ...
        if (usingNSAffineTransform)
        {
            ...
            // update the filter (using undo-compatible glue code)
            [master setFilter:filter value:tr forKey:key];
            // and set up the undo string based on the filter and key names
//            [filterStack setActionNameForFilter:filter key:key];
        }
        else
        {
            ...
            // update the filter (using undo-compatible glue code)
            [master setFilter:filter value:t forKey:key];
            // and set up the undo string based on the filter and key names
//            [filterStack setActionNameForFilter:filter key:key];
        }
    }
...
- (IBAction)stretchReadoutTextFieldChanged:(id)sender
{
    ...
    if (stretchReadoutTextField != nil && stretchSlider != nil)
    {
        ...
        if (usingNSAffineTransform)
        {
            ...
            [master setFilter:filter value:tr forKey:key];
            // and set up the undo string based on the filter and key names
//            [filterStack setg:filter key:key];
        }
        else
        {
            t = standard_fields_to_transform_vector(lastScaleFloatValue, lastAngleFloatValue, lastStretchFloatValue, lastSkewFloatValue, tx, ty);
...
- (void)addSliderForFilter:(CIFilter *)f key:(NSString *)k displayView:(NSView *)v master:(DBFilterStack *)m
{	
    float hi, lo, def;
...
- (void)addCheckBoxForFilter:(CIFilter *)f key:(NSString *)k displayView:(NSView *)v master:(DBFilterStack *)m
{
    BOOL b;
    NSCellStateValue state;
    NSRect sRect;
...
- (void)addColorWellForFilter:(CIFilter *)f key:(NSString *)k displayView:(NSView *)v master:(DBFilterStack *)m
{
    ...
    cRect = NSMakeRect(60, 5, 38, 23);
    tRect = NSMakeRect(0, 4, 56, 16);
...
- (void)addVectorForFilter:(CIFilter *)f key:(NSString *)k displayView:(NSView *)v master:(DBFilterStack *)m
{
...
- (void)addOffsetForFilter:(CIFilter *)f key:(NSString *)k displayView:(NSView *)v master:(DBFilterStack *)m
{
    int left, right, tbwid;
    float def, textbox_width;
...
- (void)addTransformForFilter:(CIFilter *)f key:(NSString *)k displayView:(NSView *)v master:(DBFilterStack *)m
    {
...
// given the filter instance and the key it's bound to
- (void)addImageWellForFilter:(CIFilter *)f key:(NSString *)k displayView:(NSView *)v master:(DBFilterStack *)m
{
...
/*- (void)addImageWellForImage:(CIImage *)im tag:(int)tag displayView:(NSView *)v master:(DBFilterStack *)m
{
    NSRect cRect, tRect, pbRect, bounds;
...
/*- (void)addTextViewForString:(NSMutableDictionary *)d key:(NSString *)k displayView:(NSView *)v master:(DBFilterStack *)m
{
    NSRect bounds, tRect;
    NSTextStorage *ts;
...
- (void)addSliderForText:(NSMutableDictionary *)d key:(NSString *)k lo:(float)lo hi:(float)hi displayView:(NSView *)v master:(DBFilterStack *)m
{
    float def;
    NSRect sRect, tRect, rRect, bounds;
    NSString *label;
