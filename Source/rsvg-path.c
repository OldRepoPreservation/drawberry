...
void (* lineToCallback) (void * sender, float x, float y);
void (* moveToCallback) (void * sender, float x, float y);
void (* curveToCallback) (void * sender, float x1, float y1, float x2, float y2, float x3, float y3);
void (* closeCallback) (void * sender);
...
rsvg_parse_path_do_cmd (RSVGParsePathCtx * ctx, gboolean final)
{
    ...
    case 'c':
        ...
        if (ctx->param == 6 || final) {
            ...
            x3 = ctx->params[4];
            y3 = ctx->params[5];
            (* curveToCallback) (ctx->sender, x1, y1, x2, y2, x3, y3);
            ctx->rpx = x2;
            ctx->rpy = y2;
            ctx->cpx = x3;
            ctx->cpy = y3;
    ...
    case 'h':
        /* horizontal lineto */
        if (ctx->param == 1) {
            (* lineToCallback) (ctx->sender, ctx->params[0], ctx->cpy);
            ctx->cpx = ctx->rpx = ctx->params[0];
            ctx->param = 0;
        }
        break;
    case 'v':
        /* vertical lineto */
        if (ctx->param == 1) {
            (* lineToCallback) (ctx->sender, ctx->cpx, ctx->params[0]);
...
void
rsvg_parse_path (const char *path_str, void *sender)
{
